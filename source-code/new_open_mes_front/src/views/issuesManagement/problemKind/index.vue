  ...
  >
    <el-form ref="ruleFormRef" :model="ruleFormData" :rules="rules" label-width="130px" status-icon>
      <el-row>
        ...
        <el-col :span="8">
          <el-form-item label="名称" :prop="isShowView ? '' : 'subTopic'">
            <el-input
              v-model="ruleFormData.subTopic"
              :disabled="isShowView"
              placeholder="请输入子主题"
            />
          </el-form-item>
...
import {
  ...
  queryFormInfo,
  deleteFormInfo
} from '@/api/issuesManagement/problemKind'

import { searchConditions, tablePurchaseColumns } from './data'
const ruleFormRef = ref<FormInstance>()
const rules = reactive<FormRules>({
  name: [{ required: true, message: '方法名称不能为空', trigger: 'change' }],
  subTopic: [{ required: true, message: '方法描述不能为空', trigger: 'change' }]
})
