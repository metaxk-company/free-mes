<template>
  ...
  >
    <el-form ref="ruleFormRef" :model="ruleFormData" :rules="rules" label-width="130px" status-icon>
      ...
      <el-row>
        <el-col>
          <el-form-item label="零部件描述" :prop="isShowView ? '' : 'description'">
            <el-input
...
import {
  ...
  queryFormInfo,
  deleteFormInfo
} from '@/api/issuesManagement/problemParts'

import { searchConditions, tablePurchaseColumns } from './data'
const ruleFormRef = ref<FormInstance>()
const rules = reactive<FormRules>({
  name: [{ required: true, message: '零部件名称不能为空', trigger: 'change' }],
  description: [{ required: true, message: '零部件描述不能为空', trigger: 'change' }]
})
