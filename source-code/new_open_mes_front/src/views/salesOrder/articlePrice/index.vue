<template>
  ...
  >
    <el-form ref="ruleFormRef" :model="ruleFormData" :rules="rules" label-width="90px" status-icon>
      <el-form-item label="物料类别" :prop="isShowView ? '' : 'category'">
        <el-select v-model="ruleFormData.category" placeholder="请选择物料类别">
          <el-option
      ...
      <el-form-item label="价格(吨）" :prop="isShowView ? '' : 'priceTon'">
        <el-input-number
          :min="0"
          v-model="ruleFormData.priceTon"
          :disabled="isShowView"
          @change="handleConvert(ruleFormData.priceTon)"
        />
      </el-form-item>
      <el-form-item label="价格(千克)">
        <el-input v-model="ruleFormData.priceKg" disabled />
      </el-form-item>
    </el-form>
    <template #footer>
...
import {
  ...
  deleteFormInfo,
  downloadListData
} from '@/api/salesOrder/articlePrice'
const ruleFormRef = ref<FormInstance>()
const rules = reactive<FormRules>({
  category: [{ required: true, message: '请选择物料类别', trigger: 'blur' }],
  priceTon: [{ required: true, message: '请输入价格(吨)', trigger: 'blur' }],
...
const ruleFormData = ref<any>({ priceTon: 0 }) // 表单数据
...
const handleConvert = (value) => {
  ruleFormData.value.priceKg = value / 1000
}
...
const handleDownload = async () => {
  ...
  download.excel(data, '物料价格.xls')
}

// 按钮新增功能
const handleOpenSave = () => {
  dialogTitle.value = '新增物料价格'
...
// 类别数据
const getKindSelectListAPI = () => {
  getKindSelectList().then((res) => {
    categoryList.value = res
    searchConditions[0].options = res.map((item) => {
      return {
        label: item.label,
        value: item.label
      }
