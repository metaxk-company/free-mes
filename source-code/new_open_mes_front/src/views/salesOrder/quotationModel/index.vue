<template>
  ...
  >
    <el-form ref="ruleFormRef" :model="ruleFormData" :rules="rules" label-width="90px" status-icon>
      <el-form-item label="型号名称" :prop="isShowView ? '' : 'model'">
        <el-input type="textarea" v-model="ruleFormData.model" :disabled="isShowView" />
      </el-form-item>
    </el-form>
    <template #footer>
...
import {
  ...
  deleteFormInfo,
  downloadListData
} from '@/api/salesOrder/quotationModel'

const ruleFormRef = ref<FormInstance>()
const rules = reactive<FormRules>({
  model: [{ required: true, message: '请输型号名称', trigger: 'blur' }]
})
...
const handleDownload = async () => {
  ...
  download.excel(data, '报价单型号.xls')
}

// 按钮新增功能
const handleOpenSave = () => {
  dialogTitle.value = '新增报价单型号'
  dialogVisible.value = true
}

// 按钮/表格 - 修改功能
const handleOpenEdit = async (value?) => {
  dialogTitle.value = '修改报价单型号'
