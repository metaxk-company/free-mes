          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item @click="handleIncludeTaxPrint('hanshui', scope.row)"
                >含税</el-dropdown-item
              >
              <el-dropdown-item @click="handleIncludeTaxPrint('wushui', scope.row)"
                >无税</el-dropdown-item
              >
              <el-dropdown-item @click="handleIncludeTaxPrint('wujiage', scope.row)"
                >无价格</el-dropdown-item
              >
              <el-dropdown-item @click="handleIncludeTaxPrint('zhiling', scope.row)"
                >指令</el-dropdown-item
              >
              <el-dropdown-item @click="handleIncludeTaxPrint('bangmadan', scope.row)"
                >磅码单</el-dropdown-item
              >
            </el-dropdown-menu>
          </template>
        </el-dropdown>
      ...
      <el-row>
        ...
        <el-col :span="6">
          <el-form-item label="总净重">
            {{ (ruleFormData.totalWeight = getTotalNetWeight) }}
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="出库来源" :prop="isShowView ? '' : 'source'">
            {{ ruleFormData.source }}
            <!-- <el-select placeholder="请选择出库来源" v-model="ruleFormData.source">
              <el-option
        ...
        <el-col :span="6">
          <el-form-item label="出库状态" :prop="isShowView ? '' : 'status'">
            {{ ruleFormData.status }}
            <!-- <el-select placeholder="请选择出库状态" v-model="ruleFormData.status">
              <el-option
...
import { confirmSync } from '@/api/a_public_port'
...
const handleUpDataList = (value) => {
  ...
  // // 总件数
  // tableReturnList.value[value.index].pieces = value.length
}
// 确认同步
...
const calculateTotal = (property) => {
  return computed(() => {
    return tableReturnList.value.reduce((accumulator, currentValue) => {
      return ((accumulator + Number(currentValue[property]) || 0) * 1000) / 1000
    }, 0)
  })
}
...
const _getSendOutTotal = computed(() => {
  return tableReturnList.value.reduce((accumulator, currentValue) => {
    ...
    return (ruleFormData.value.sendOut = accumulator + height)
  }, 0)
})
...
const getTotalPriceAmount = computed(() => {
  return tableReturnList.value
    .reduce((accumulator, currentValue) => {
      const height = Number(currentValue.totalPrice) || 0
      return (ruleFormData.value.outboundTotalPrice = accumulator + height)
    }, 0)
    .toFixed(2)
})
...
const handleAddPurchase = () => {
  ...
  ruleFormData.value.saleNumber = selectForm.saleNumber
  ruleFormData.value.deliveryDate = selectForm.deliveryDate
  ruleFormData.value.orderDate = selectForm.orderDate
  ruleFormData.value.customerNumber = selectForm.customerNumber
  ruleFormData.value.customerOrderNumber = selectForm.customerOrderNumber
  ruleFormData.value.quantity = selectForm.quantity
  ruleFormData.value.totalPrice = selectForm.totalPrice
  ruleFormData.value.sendOut = selectForm.sendOut
  ruleFormData.value.noSend = selectForm.noSend
  ruleFormData.value.customerName = selectForm.customerName
  ruleFormData.value.isTax = selectForm.isTax
  ruleFormData.value.saleTotalPrice = selectForm.totalPrice
  // 清空当前已选择的采购数据
...
const handleDownload = async () => {
  ...
  download.excel(data, '销售出库-有销售单.xls')
}
// 按钮新增功能
...
const getClientSaleTableList = () => {
  ...
  queryClientSaleTableList(params).then((res) => {
    purchaseDataList.value = res.list
    paginationPurchase.total = res.total
  })
}
