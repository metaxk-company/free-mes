<template>
  ...
  >
    <el-form ref="ruleFormRef" :model="ruleFormData" :rules="rules" label-width="100px" status-icon>
      <el-row>
        <el-col :span="6">
          <el-form-item label="库区编码" :prop="isShowView ? '' : 'areaNumber'">
            {{ ruleFormData.areaNumber }}
            <!-- <el-input v-model="ruleFormData.areaNumber" :disabled="isShowView" /> -->
          </el-form-item>
        </el-col>
        <!-- <el-col :span="6">
        ...
        <el-col :span="6">
          <el-form-item label="库区名称" :prop="isShowView ? '' : 'areaName'">
            <el-input v-model="ruleFormData.areaName" :disabled="isShowView" />
          </el-form-item>
        </el-col>
        <el-col :span="6">
...
import {
  ...
  updateFormInfo,
  deleteFormInfo
} from '@/api/storeHouse/wareHouseArea'
...
const rules = reactive<FormRules>({
  areaNumber: [{ required: true, message: '请输入库区编码', trigger: 'blur' }],
...
const handleCreateCode = () => {
  getCreateCode('LOCATION_CODE').then((res) => {
    ruleFormData.value.areaNumber = res
  })
}
