<template>
  ...
  >
    <el-form ref="ruleFormRef" :model="ruleFormData" :rules="rules" label-width="100px" status-icon>
      <el-row>
        <el-col :span="6">
          <el-form-item label="仓库编码" :prop="isShowView ? '' : 'warehouseNumber'">
            {{ ruleFormData.warehouseNumber }}
            <!-- <el-input v-model="ruleFormData.warehouseNumber" :disabled="isShowView" /> -->
          </el-form-item>
        </el-col>
        <!-- <el-col :span="6">
        ...
        <el-col :span="6">
          <el-form-item label="仓库名称" :prop="isShowView ? '' : 'warehouseName'">
            <el-input v-model="ruleFormData.warehouseName" :disabled="isShowView" />
          </el-form-item>
        </el-col>
        <el-col :span="6">
...
import {
  ...
  updateFormInfo,
  deleteFormInfo
} from '@/api/storeHouse/warehouseModel'
import { getCreateCode } from '@/api/a_public_port'

const ruleFormRef = ref<FormInstance>()
const rules = reactive<FormRules>({
  warehouseNumber: [{ required: true, message: '请输入仓库编码', trigger: 'blur' }],
...
const handleCreateCode = () => {
  getCreateCode('WAREHOUSE_CODE').then((res) => {
    ruleFormData.value.warehouseNumber = res
  })
}
